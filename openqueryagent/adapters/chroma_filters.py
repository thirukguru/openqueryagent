"""Chroma filter compiler.

Compiles universal FilterExpression objects into Chroma's ``where`` and
``where_document`` filter dictionary formats.
"""
...
# Map universal operators to Chroma operators
...
# Operators not natively supported by Chroma
...
class ChromaFilterCompiler:
    """Compile FilterExpression → Chroma where/where_document dicts."""
    ...
    def compile(
        ...
        """Compile a FilterExpression into Chroma filter dict."""
    ...
    def _compile_node(self, expr: FilterExpression) -> dict[str, Any]:
        ...
        if expr.operator in _UNSUPPORTED:
            msg = f"Operator {expr.operator} is not supported by Chroma"
        ...
        if expr.operator == FilterOperator.CONTAINS:
            return {"$contains": expr.value}

        chroma_op = _OP_MAP.get(expr.operator, "$eq")
        field = expr.field or ""
        return {field: {chroma_op: expr.value}}
    ...
    def _validate_node(
        ...
        if expr.operator in _UNSUPPORTED:
            errors.append(FilterValidationError(
                field=expr.field or "",
                operator=str(expr.operator),
                message=f"Operator {expr.operator} not supported by Chroma",
            ))
